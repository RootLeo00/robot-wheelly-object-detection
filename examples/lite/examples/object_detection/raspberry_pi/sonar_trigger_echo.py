...
class ranger:
   ...
   def __init__(self, pi, trigger, echo):
      ...
      # callback for the sonar pin
      self._cb = pi.callback(self._trig, pigpio.EITHER_EDGE, self._callbackfunction)
      self._cb = pi.callback(self._echo, pigpio.EITHER_EDGE, self._callbackfunction)
   ...
   def read(self):
      ...
      round trip cm = round trip time / 1000000.0 * 34300
      """
      if self._inited:
         self._ping = False
...
if __name__ == "__main__":
   ...
   sonar = sonar_trigger_echo.ranger(pi, 25, 22)
